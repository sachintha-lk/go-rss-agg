...
func scrapeFeed(db *database.Queries, wg *sync.WaitGroup, feed database.Feed) {
	...
	if err != nil {
		log.Println("Error marking feed as featched:", err)
		return
	}
	...
	for _, item := range rssFeed.Channel.Item {
		log.Println("Found Post", item.Title, "on feed", item.Link, "url on feed", feed.Name)
		...
		if err != nil {
			...
			log.Printf("couldn't save the post %v with err %v", item.Title, err)
			continue
		}
	}
